...
export async function updateConfigFilesForEnvironment(environment, configFile) {
  const kinstaConfigFile = yaml.load(fs.readFileSync(configFile, "utf8"))
  const trellisPath = getTrellisPath()
  const appName = await getAppName()
  const host = `kinsta_${environment} ansible_host=${kinstaConfigFile[environment].ansible_host} ansible_ssh_port=${kinstaConfigFile[environment].ansible_ssh_port} ansible_ssh_extra_args='-o StrictHostKeyChecking=no'
...
kinsta_${environment}`
  ...
  const { sitename } = kinstaConfigFile
  const apiDomain = kinstaConfigFile[environment].canonical
  ...
  const main = `project_root: ${kinstaConfigFile[environment].project_root}
