...
module.exports = async (url, opts) => {
  ...
  const attemptsNum = ++retriesLeft // add first request to attemps
  ...
  if (opts.retryOnHttpResponse == undefined) {
    ...
    if (opts.retryOnHttpResponse === false) {
      ...
      throw new Error(
        `'retryOnHttpResponse' must be a function: ${opts.retryOnHttpResponse}`,
      )
    }
  }

  if (
    opts.beforeRetry !== undefined &&
    typeof opts.beforeRetry !== "function"
  ) {
  ...
  while (retriesLeft > 0) {
    ...
    try {
      ...
      if (opts.beforeRetry) {
        const newOpts = opts.beforeRetry(attemptsNum - retriesLeft, e)
        if (newOpts) {
