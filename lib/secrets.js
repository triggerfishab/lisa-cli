...
export async function generateSecrets(initialProjectName = "") {
  ...
  const jwtSecret = generator.generate({
    length: 32,
    numbers: true,
  })

  const apiSecret = generator.generate({
    length: 32,
    numbers: true,
  })

  const apiRevalidateSecret = generator.generate({
    length: 32,
    numbers: true,
  })
  ...
  allVaultFile.vault_wordpress_env_defaults.next_api_revalidate_secret =
    apiRevalidateSecret
  ...
  await exec(
    `printf ${apiUrl} | vercel env add NEXT_PUBLIC_API_URL development`,
    { cwd: appName },
  )
  ...
  await exec(
    `printf "${apiRevalidateSecret}" | vercel env add API_REVALIDATE_SECRET production`,
    { cwd: appName },
  )
  await exec(
    `printf "${apiRevalidateSecret}" | vercel env add API_REVALIDATE_SECRET preview`,
    { cwd: appName },
  )
  await exec(
    `printf "${apiRevalidateSecret}" | vercel env add API_REVALIDATE_SECRET development`,
    { cwd: appName },
  )

  writeSuccess("API_REVALIDATE_SECRET environment variable saved to Vercel.")
  ...
  writeSuccess(".env created with environments variables from Vercel.")
}
