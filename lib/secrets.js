...
const { getGroupVarsPath, getTrellisPath } = require("./trellis")
const fs = require("fs")
const yaml = require("js-yaml")
const exec = require("../lib/exec")
const generator = require("generate-password")
const { getValetTld } = require("../tasks/valet")
const { writeSuccess, writeInfo, writeStep } = require("./write")
...
async function generateSecrets() {
  ...
  writeStep("Installing WordPress, please enter some information below.")

  let tld = await getValetTld()
  let apiUrl = `https://${apiName}.${tld}`

  await exec(
    `wp core install --url=${apiUrl} --title="Lisa" --admin_user=triggerfish_root --admin_password=admin --admin_email=signup@triggerfish.se --skip-email`,
    {
      cwd: `${apiName}/site`,
    }
  )
